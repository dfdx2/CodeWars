/* 7KYU CodeWars I will take take the biggest one and nothing else

We have a positive integer N, N > 0.

We have a special integer N1, N1 > N, such that N1 = c ‧ d and N = c + d 
...
class Kata {
  public static long IWillTakeTheBiggestOneAndNothingElse(long n) {
...
class Kata {
  public static long maxIntChain(long n) {
    ...
    return (long) Math.floor(n / 2.0 + 1) * (long) Math.ceil(n / 2.0 - 1);
  }
}
----------------------------------------------------------------------
class Kata {
...
class Kata {
  public static long maxIntChain(long n) {
    return n > 4 ? (long) Math.floor(n / 2.0 + 1) * (long) Math.ceil(n / 2.0 - 1) : -1;
  }
}
-----------------------------------------------------------------------
class Kata {
  public static long maxIntChain(long n) {
   return n < 5 ? -1 : (n % 2 == 0 ? (n / 2 + 1) * (n / 2 - 1) : (n / 2 + 1) * (n / 2));
  }
}
-----------------------------------------------------------------------
public class Kata {
  public static long maxIntChain(long n) {
    long result = -1;
    if (n >= 5) {
      long upperBound = (n / 2) - (n % 2) + 1;
      result = upperBound * (n - upperBound);
    }
    return result;
  }
}
