/* 7KYU CodeWars Growing Plant

Task
Each day a plant is growing by upSpeed meters. Each night 
that plant's height decreases by downSpeed meters due to 
the lack of sun heat. Initially, plant is 0 meters tall. 
We plant the seed at the beginning of a day. We want to 
know when the height of the plant will reach a certain level.

Example

For upSpeed = 100, downSpeed = 10 and desiredHeight = 910, the 
output should be 10.
...
public class SimpleFun {
    public static int growingPlant(int upSpeed, int downSpeed, int desiredHeight) {
        int res = (int) Math.ceil((double)(desiredHeight - downSpeed) / (upSpeed - downSpeed));
        return res > 0 ? res : 1;
    }
}
-------------------------------------------------------------------
...
public class SimpleFun {
    public static int growingPlant(int upSpeed, int downSpeed, int desiredHeight) {
        int grow = 0;
        int days = 1;
        while (grow + upSpeed < desiredHeight){
            days++;
            grow = (grow + upSpeed) - downSpeed;
        }
        return days;
    }
}
...
public class SimpleFun {
    public static int growingPlant(int u, int d, int m) {
        return (int) Math.max(1, Math.ceil((double) (m - d) / (u - d)));
    }
}
*/