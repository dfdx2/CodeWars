/* &KYU CodeWars Count All The Sheep On A Farm In The Heights Of New Zealand

Every Friday and Saturday night, farmer counts amount of sheep returned back 
to his farm (sheep returned on Friday stay and don't leave for a weekend).
...
Farmer knows the total amount of sheep, this is a third parameter. You need to 
return the amount of sheep lost (not returned to the farm) after final sheep 
...
public class Kata {
  public static int lostSheep(int[] fridayNightCounting, int[] saturdayNightCounting, int sheepTotal) {
    ...
    for (int i = 0; i < fridayNightCounting.length; i++) {
      result += fridayNightCounting[i];
    }
    for (int i = 0; i < saturdayNightCounting.length; i++) {
      result += saturdayNightCounting[i];
    }
    return sheepTotal - result;
  }
}
/*------------------------------------------------------------------------------
